"""
MeetTranscribe — Streamlit 版（完全免費，使用 Groq API）
...
# ── 常數 ───────────────────────────────────────────────────────────────────────
WHISPER_MAX  = 24 * 1_048_576          # 24 MB（Groq Whisper 上限 25 MB）
...
def _split_audio(src: str, chunk_min: int = 8) -> list | None:
    """ffmpeg 切片，回傳 [(path, duration_sec)] 或 None"""
    if not _ffmpeg_ok():
        return None
...
def _call_whisper(client, path: str, lc: str | None) -> list[dict]:
    ...
    # 如果沒有 segment，退回到純文字
    if not out and hasattr(r, "text") and r.text:
...
def transcribe_audio(data: bytes, filename: str, groq_key: str, lc: str | None) -> list[dict]:
    ...
    try:
        ...
        if not chunks:
            raise ValueError(
                f"檔案 {size/1_048_576:.1f} MB 超過 25 MB 限制，"
                "且找不到 ffmpeg 無法自動分割。\n"
...
請根據上下文判斷並修正專業術語、公司名稱、產品名稱、人名等用詞。
...
要求：summary 2–4 小節、每節 3–6 要點並標明發言者；action_items 須指明負責人；topics 2–4 個標籤。"""
...
<div class="hero">
  ...
  <p>上傳音訊 · Groq Whisper 免費轉錄 · 指定發言者 · Llama AI 免費分析 · 一鍵輸出會議記錄</p>
</div>
""", unsafe_allow_html=True)
            ...
            if not groq_key:
                ...
                with st.spinner("Groq Whisper 轉錄中，請稍候…"):
                    try:
                        ...
                        segs = transcribe_audio(uploaded.read(), uploaded.name, groq_key, lc)
                        st.session_state.transcript = [
                            {
            ...
            if go_rec:
                if not groq_key:
                    ...
                    with st.spinner("Groq Whisper 轉錄中…"):
                        try:
                            lc   = LANG_MAP[language]
                            segs = transcribe_audio(audio_val.read(), "recording.wav", groq_key, lc)
                            st.session_state.transcript = [
                                {
                                    "speaker":     speaker_names[0],
                                    "text":        s["text"],
