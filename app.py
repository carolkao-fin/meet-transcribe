"""
MeetTranscribe — Streamlit 版
上傳音檔（最大 100 MB）→ Whisper 轉錄 → 指定發言者 → Claude AI 分析
"""
...
# ── Helpers ────────────────────────────────────────────────────────────────────
WHISPER_MAX = 24 * 1_048_576   # 24 MB (Whisper API limit is 25 MB)
...
def _split_audio(src: str, chunk_min: int = 8) -> list | None:
    """Split audio into chunks using ffmpeg. Returns [(path, duration_sec)] or None."""
    if not _ffmpeg_ok():
        return None
...
def transcribe_audio(data: bytes, filename: str, openai_key: str, lc: str | None) -> list[dict]:
    ...
    try:
        ...
        if not chunks:
            raise ValueError(
                f"檔案 {size/1_048_576:.1f} MB 超過 Whisper 25 MB 限制，"
                "且找不到 ffmpeg 無法自動分割。\n"
                "請安裝 ffmpeg（https://ffmpeg.org）或先壓縮音訊後再上傳。"
            )
...
def analyze_with_claude(transcript: list, meeting_info: dict, claude_key: str) -> dict:
...
- 時長：{meeting_info.get("duration", "")}
- 參與者：{", ".join(participants) if participants else "未知"}

逐字稿：
{txt}

請根據上下文判斷並修正專業術語、公司名稱、產品名稱、人名等用詞，使其符合實際語境。

請只回傳一個合法的 JSON 物件，格式如下（不要包含 markdown 或其他文字）：
...
{{
  ...
  "action_items": [
    {{
      ...
      "description": "這組任務的說明",
      "items": [{{"assignee": "負責人", "task": "具體任務（以動詞開頭）"}}]
    }}
  ],
...
要求：
1. summary 應有 2–4 個小節，每節 3–6 個要點，須標明發言者
2. action_items 須明確指出負責人，任務描述要具體可執行
3. corrected_transcript 須修正辨識錯誤的術語、名稱、專有名詞
4. topics 為 2–4 個詞的標籤，反映會議主要議題"""
...
with st.sidebar:
    ...
    with st.expander("🔑 API 金鑰", expanded=True):
        ...
        openai_key  = st.text_input("OpenAI API Key（Whisper）", type="password", placeholder="sk-...")
...
<div class="hero">
  ...
  <p>上傳音訊檔案 · Whisper 自動轉錄 · 指定發言者 · Claude AI 分析 · 一鍵輸出會議記錄</p>
</div>
""", unsafe_allow_html=True)
...
with tab_up:
    st.markdown(
        "支援格式：**mp3 · wav · m4a · aac · ogg · flac · webm**　｜　最大 **100 MB**"
    )
    ...
    if uploaded:
        ...
        if go:
            if not openai_key:
                st.error("請在左側輸入 OpenAI API Key")
            else:
                with st.spinner("Whisper 轉錄中，大檔案需要幾分鐘，請稍候…"):
                    try:
                        lc   = LANG_MAP[language]
                        segs = transcribe_audio(uploaded.read(), uploaded.name, openai_key, lc)
                        st.session_state.transcript = [
                            {
                                "speaker":     speaker_names[0],
                                "text":        s["text"],
                                "displayTime": secs_hms(s["start"]),
                                "rawTime":     int(s["start"] * 1000),
                            }
                            for s in segs
                        ]
                        st.session_state.analysis    = None
...
# ── Tab 2: Record ──────────────────────────────────────────────────────────────
with tab_rec:
    st.info("💡 點擊麥克風錄音，完成後點「轉錄錄音」。需要 OpenAI API Key。")
    try:
        ...
        if audio_val:
            ...
            if go_rec:
                if not openai_key:
                    ...
                    with st.spinner("Whisper 轉錄中…"):
                        try:
                            lc   = LANG_MAP[language]
                            segs = transcribe_audio(audio_val.read(), "recording.wav", openai_key, lc)
                            st.session_state.transcript = [
                                {
                                    "speaker":     speaker_names[0],
                                    "text":        s["text"],
...
if st.session_state.transcript:
    ...
    with st.expander(f"📝 逐字稿編輯　（{len(st.session_state.transcript)} 段，可修改文字 / 指定發言者）",
                     expanded=True):
        for i, entry in enumerate(st.session_state.transcript):
            ...
            with c_time:
                st.markdown(
                    f"<p style='color:#9ca3af;font-size:.8rem;padding-top:.55rem'>"
                    f"{entry['displayTime']}</p>",
                    unsafe_allow_html=True,
                )
            with c_txt:
    ...
    if do_analyze:
        if not claude_key:
            ...
            with st.spinner("Claude 分析中，請稍候…"):
                try:
                    info = {
                        "title":        meeting_title,
                        ...
                        "participants": list(dict.fromkeys(
                            e["speaker"] for e in st.session_state.transcript
                        )),
                    }
                    result = analyze_with_claude(st.session_state.transcript, info, claude_key)
...
if st.session_state.analysis:
    ...
    if data.get("action_items"):
        ...
        for grp in data["action_items"]:
            ...
            for item in grp.get("items", []):
                st.markdown(
                    f'<div class="ai-item">'
                    f'<span class="ai-who">{item["assignee"]}</span> {item["task"]}'
                    f'</div>',
                    unsafe_allow_html=True,
                )
    ...
    st.text_area("備註（可自行填寫）", height=80, label_visibility="collapsed", key="notes_ta")
